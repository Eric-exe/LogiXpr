...
bool evaluateExpression(std::shared_ptr<Expression> expression, std::unordered_map<std::string, bool> inputs) {
    ...
    if (value == XOR) return evaluateExpression(expression->getLeft(), inputs) != evaluateExpression(expression->getRight(), inputs);
    if (value == IMPLIES) return !evaluateExpression(expression->getLeft(), inputs) || evaluateExpression(expression->getRight(), inputs);
    if (value == IFF) return evaluateExpression(expression->getLeft(), inputs) == evaluateExpression(expression->getRight(), inputs);
...
bool isEquivalent(std::shared_ptr<Expression> lhs, std::shared_ptr<Expression> rhs) {
    ...
    for (auto input : inputs) {
        if (evaluateExpression(lhs, input) != evaluateExpression(rhs, input)) return false;
    }
