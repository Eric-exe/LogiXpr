...
//     std::cout << parse("!(p | (!p & q))", lhs) << std::endl;
//     parse("!p & !q", rhs);

//     // unparse and parse again, removes unnecessary parentheses
//     parse(lhs->toStringTree(), lhs);
//     parse(rhs->toStringTree(), rhs);
//     preprocess(lhs, rhs);

//     std::cout << isEquivalent(lhs, rhs) << std::endl;
//     std::vector<std::vector<std::string>> steps = proveEquivalence(lhs, rhs);
...
====================================================================)" << std::endl;
    ...
    if (parse(lhs, lhsTree)) {
        std::cout << SUCCESS << "Parsed left-hand side expression successfully!" << std::endl;
    } else {
        std::cout << FAILURE << "Failed to parse left-hand side expression :(" << std::endl;
        return 1;
    }

    if (parse(rhs, rhsTree)) {
        std::cout << SUCCESS << "Parsed right-hand side expression successfully!" << std::endl;
    } else {
        std::cout << FAILURE << "Failed to parse right-hand side expression :(" << std::endl;
        return 1;
    }
    ...
    if (parse(lhsTree->toStringTree(), lhsTree)) {
        std::cout << SUCCESS << "Removed unnecessary parentheses from left-hand side expression!" << std::endl;
    } else {
        std::cout << FAILURE << "Failed to remove unnecessary parentheses from left-hand side expression :(" << std::endl;
        return 1;
    }

    if (parse(rhsTree->toStringTree(), rhsTree)) {
        std::cout << SUCCESS << "Removed unnecessary parentheses from right-hand side expression!" << std::endl;
    } else {
        std::cout << FAILURE << "Failed to remove unnecessary parentheses from right-hand side expression :(" << std::endl;
        return 1;
    }
    ...
    std::cout << "====================================================================" << std::endl;
    std::vector<std::vector<std::string>> steps = proveEquivalence(lhsTree, rhsTree);
    ...
    for (auto step : steps) {
        ...
        else if (processedSteps.size() > 0 && processedSteps.back()[0] != processedStr)
            processedSteps.push_back({processedStr, step[1]});
    }
    ...
    std::cout << header << std::string(headerPadding, ' ') << "Justification" << std::endl;
    std::cout << std::endl;
    for (auto step : processedSteps) {
        std::cout << step[0] << step[1] << std::endl;
    }
